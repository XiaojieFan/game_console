var page = {
    ...
    onLoad: function(event) {
        var data = new Object();
        for (var btn in this.keyboard) {
            data[btn] = { customProperty: this.keyboard[btn] };
        }
        this.setData(data);
        this.freshKeyboard();
        if (event) {
    ...
    onBtn: function(event) {
        switch (event.target.id) {
            ...
            default:
                if (this.isLetter) {
                    if (this.isCaps) {
                        this.text += event.detail.customProperty.letterCaps;
                    } else {
                        this.text += event.detail.customProperty.letter;
                    }
                } else {
                    if (this.isCaps) {
                        this.text += event.detail.customProperty.symbolCaps;
                    } else {
                        this.text += event.detail.customProperty.symbol;
                    }
                }
                this.setData({ TextLabel: { value: this.text, refresh: true } })
                break;
